...
# HGF= Highway Miles of Fuel consumed

dataset=read.csv('VMTvsHGF2.csv')

#Creating Scatter Plot between VMT (x) and HGF (y)

library(ggplot2)
scatter<-ggplot(dataset, aes(TVMT, THGF))
...
scatter+geom_point()+labs(x="Million Vehicle Miles Traveled", y="Million Highway Gallons of Fuel")+
...
regressor<-lm(formula=THGF~ TVMT, data<-dataset) 
...
#Residuals Analysis Plots: Extended version.
# library(lindia)
